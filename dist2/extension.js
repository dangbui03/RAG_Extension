...
// src/utils/utils.ts
async function GetOllamaModelFromUser(models) {
  let model = getConfiguration("modelName");
  if (model === "" || model === void 0) {
    if (models.length === 0) {
    ...
    if (selectedModel === void 0 || selectedModel === "") {
...
function getConfiguration(section, defaultValue = "", setting = "ollama") {
...
function handleError(error) {
  ...
  } else if (error?.type === 2 /* ERROR */) {
    vscode.window.showErrorMessage(error?.message);
  } else if (error?.type === 3 /* WARNING */) {
...
var PromptBuilder = class {
  ...
  buildFunctionPrompt() {
    ...
    let prompt = getConfiguration("prompt").replace("{language}", language).replace("{codeblock}", "function");
    prompt = `
${prompt}
`;
  ...
  buildContext() {
    if (getConfiguration("giveContext")) {
      let context = this.editor.getEditorContent();
...
`;
  ...
  buildPromptText() {
    let language = this.editor.getLanguage();
    let prompt = getConfiguration("prompt").replace("{language}", language);
    prompt = `
${prompt}
`;
    this.prompt.setPromptText(prompt);
  ...
  buildCodeBlock(codeBlock) {
    if (codeBlock === "" || codeBlock === void 0) {
      throw new MyError("please select code", 1 /* INFO */);
    }
...
`;
  ...
  build() {
    let context = this.prompt.getContext();
    let promptText = this.prompt.getPromptText();
    let codeBlock = this.prompt.getCodeBlock();
...
var Ollama2 = class extends Ollama$1 {
  ...
  async createBlob(path2) {
    ...
    const fileStream = (0, import_fs.createReadStream)(path2);
...
// src/core/prompts/ollama.ts
var import_cross_fetch = __toESM(require_node_ponyfill());
var OllamaServer = class _OllamaServer {
  ollama;
  static server;
  constructor(url) {
    this.ollama = new Ollama2({ host: url, fetch: import_cross_fetch.default });
  }
  static getInstance(hostUrl) {
    if (!_OllamaServer.server) {
      _OllamaServer.server = new _OllamaServer(hostUrl);
    }
    return _OllamaServer.server;
  }
  async listModels() {
    let list = await this.ollama.list();
    let models = [];
  ...
  async generateComment(model, prompt) {
...
// src/core/editor/manageEditor.ts
...
var ActiveEditor = class {
  ...
  getActiveEditor() {
    ...
    if (editor === void 0) {
      throw new MyError("Failed to retrieve editor", 2 /* ERROR */);
    }
    return editor;
  }
  getLanguage() {
  ...
  getSelection() {
    ...
    const codeBlock = this.editor.document.getText(selection);
    return codeBlock;
  }
  async getLineNumber(selection) {
    const startLine = selection.start.line;
    let lineAbove = selection.start.line - 1;
    if (lineAbove < 0) {
      const position = new vscode2.Position(0, 0);
      ...
      await this.editor.edit((editBuilder) => {
        const position = new vscode2.Position(startLine, 0);
        editBuilder.insert(position, "\n");
        lineAbove = lineAbove + 1;
      });
    }
  ...
  async getAllSymbols() {
    const allsymbols = await vscode2.commands.executeCommand("vscode.executeDocumentSymbolProvider", this.document.uri);
    if (allsymbols.length === 0) {
      throw new MyError("No Symbol found", 1 /* INFO */);
    }
    return allsymbols;
  }
  async getAllFunctionName() {
    let allFunctions = new Array();
    let allSymbols = await this.getAllSymbols();
    for (let symbol of allSymbols) {
      if (symbol.kind === vscode2.SymbolKind.Function) {
  ...
  async getFunction(func) {
    let allSymbols = await this.getAllSymbols();
    for (let symbol of allSymbols) {
      if (symbol.kind === vscode2.SymbolKind.Function && symbol.name === func) {
        return symbol;
      }
    }
    throw new MyError("Function not found ", 2 /* ERROR */);
  }
  async getSelectedFunction(func) {
    let symbol = await this.getFunction(func);
    let functionString = this.editor.document.getText(symbol.range);
    return functionString;
  }
  async addCommentToFile(generatedComment, selection) {
    ...
    edit.insert(fileURI, new vscode2.Position(lineNumber, 1), generatedComment.trim() + "\n");
    await vscode2.workspace.applyEdit(edit);
    vscode2.window.showInformationMessage(`Comment added to ${fileName} at line ${lineNumber + 1}`);
  }
};
...
async function GenerateCommentCommand() {
  vscode3.window.showInformationMessage("Generate Comment Command Executed!");
  try {
    const editor = new ActiveEditor();
    const serverUrl = getConfiguration("serverURL", "http://127.0.0.1:11434");
    const ollamaServer = OllamaServer.getInstance(serverUrl);
    const models = await ollamaServer.listModels();
    const model = await GetOllamaModelFromUser(models);
    const promptbuilder = new PromptBuilder(editor);
    let prompt = promptbuilder.buildContext().buildPromptText().buildCodeBlock(editor.getSelection()).build();
    const fullPrompt = prompt.getFullPrompt();
    console.log("Generated Prompt:", fullPrompt);
    const comment = await ollamaServer.generateComment(model, fullPrompt);
    ...
    handleError(error);
    vscode3.window.showErrorMessage("Failed to generate comments. Please check the logs for more details.");
  }
}

// src/commands/functionComment.ts
var vscode4 = __toESM(require("vscode"));
async function GfunctionCommentCommand() {
  vscode4.window.showInformationMessage("Gfunction Comment Command Executed!");
  try {
    let editor = new ActiveEditor();
    let selectedfunction = await vscode4.window.showQuickPick(editor.getAllFunctionName(), {
    ...
    const serverUrl = getConfiguration("serverURL", "http://127.0.0.1:11434");
    const ollamaServer = OllamaServer.getInstance(serverUrl);
    const model = await GetOllamaModelFromUser(await ollamaServer.listModels());
    const promptBuilder = new PromptBuilder(editor);
    const codeBlock = await editor.getSelectedFunction(selectedfunction);
    let prompt = promptBuilder.buildContext().buildFunctionPrompt().buildCodeBlock(codeBlock).build();
    let fullPrompt = prompt.getFullPrompt();
    console.log(fullPrompt);
    let symbol = await editor.getFunction(selectedfunction);
    let line = symbol.range.start.line;
    let comment = await ollamaServer.generateComment(model, fullPrompt);
...
// src/commands/readEntireCodeBase.ts
var vscode6 = __toESM(require("vscode"));
var import_fs2 = __toESM(require("fs"));
var import_path2 = __toESM(require("path"));

// src/core/explorer/ExplorerProvider.ts
...
var ExplorerProvider = class {
  ...
  getChildren() {
    if (!vscode5.workspace.workspaceFolders) {
      return Promise.resolve([]);
    }
  ...
  getFilesAndFolders(rootPath) {
    ...
    const filesAndFolders = fs3.readdirSync(rootPath).map((file) => {
      const fullPath = path2.join(rootPath, file);
      const isDirectory = fs3.statSync(fullPath).isDirectory();
      const label = file;
      return new vscode5.TreeItem(
        label,
...
function catchNextJsVersion() {
  if (!vscode6.workspace.workspaceFolders) {
    vscode6.window.showWarningMessage("No workspace is open.");
    return;
  }
  const workspaceRoot = vscode6.workspace.workspaceFolders[0].uri.fsPath;
  const targetFiles = ["package.json", "tsconfig.json"];
  let nextJsVersion = null;
  targetFiles.forEach((fileName) => {
    const filePath = import_path2.default.join(workspaceRoot, fileName);
      ...
      if (fileName === "package.json") {
        try {
          ...
          if (packageJson.dependencies && packageJson.dependencies["next"]) {
            ...
            vscode6.window.showInformationMessage(`Next.js Version: ${nextJsVersion}`);
          } else {
            vscode6.window.showInformationMessage("Next.js not found in package.json dependencies.");
          }
        } catch (error) {
          vscode6.window.showErrorMessage("Failed to parse package.json.");
        }
      } else {
        vscode6.window.showInformationMessage(`Content of ${fileName}: ${content}`);
      }
    } else {
      vscode6.window.showWarningMessage(`${fileName} not found in the project.`);
    }
  });
...
async function createOrUpdateFile(fileName, content) {
  ...
  const filePath = vscode6.Uri.joinPath(workspaceRoot, fileName);
  try {
    await vscode6.workspace.fs.writeFile(filePath, Buffer.from(content, "utf8"));
    vscode6.window.showInformationMessage(`${fileName} created/updated successfully!`);
  } catch (error) {
    vscode6.window.showErrorMessage(`Failed to create/update ${fileName}: ${error}`);
  }
}
async function readEntireCodeBase() {
  vscode6.window.showInformationMessage("Read Entire Code Base Command Executed!");
  try {
    if (!vscode6.workspace.workspaceFolders) {
      vscode6.window.showErrorMessage("No workspace is open.");
      return;
    }
    const explorer = vscode6.window.createTreeView("explorer", { treeDataProvider: new ExplorerProvider() });
    readSpecificFiles();
    const version2 = catchNextJsVersion();
    vscode6.window.showInformationMessage(`Next.js Version: ${version2}`);
    await createOrUpdateFile("new-config.json", JSON.stringify({ key: "value" }, null, 2));
  } catch (err) {
    handleError(err);
    vscode6.window.showErrorMessage("Failed to read entire code base. Please check the logs for more details.");
  }
}

// src/core/webview/RagginProvider.ts
var vscode7 = __toESM(require("vscode"));

// src/utils/generator.ts
async function generateAnswer(question, model, webview) {
  try {
    var answer = "";
    var augmented_question = question;
    const output = await index.generate({
      model,
    ...
    for await (const chunk of output) {
      ...
      webview.postMessage({ type: "update", model, content: answer });
    }
...
function replaceWebviewHtmlTokens(html, options) {
  const { cssUri, cspNonce, jsUri } = options;
  return html.replace(
    /#{(cssUri | cspNonce)} /g,
    (_substring, token) => {
      switch (token) {
...
var RagginProvider = class {
  constructor(_extensionUri, _context, viewName) {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        ...
        case "populateModels": {
          const serverUrl = getConfiguration(
            "serverURL",
  ...
  async getWebviewsUri() {
    return vscode7.Uri.joinPath(await this.getRootUri(), "webviews-ui");
  }
  async _getViewJSUri(webview) {
    return webview.asWebviewUri(
      vscode7.Uri.joinPath(await this.getWebviewsUri(), this.viewName, `${this.viewName}.js`)
    );
  }
  async _getHtmlForWebview(webview) {
    const cssUri = webview.asWebviewUri(
      vscode7.Uri.joinPath(await this.getRootUri(), "dist", "output.css")
    );
    ...
    try {
      const [bytes] = await Promise.all([vscode7.workspace.fs.readFile(uri)]);
      const htmlContent = utf8TextDecoder.decode(bytes);
      const html = replaceWebviewHtmlTokens(htmlContent, {
        cssUri: cssUri.toString(),
        jsUri: this._getViewJSUri(webview).toString(),
        cspNonce: getNonce()
      });
      return html;
    } catch (error) {
...
function activate(context) {
  ...
  context.subscriptions.push(
    new vscode8.Disposable(() => vscode8.Disposable.from(...disposables).dispose())
  );
  ...
  item.show();
  context.subscriptions.push(
    vscode8.window.registerWebviewViewProvider("raggin-sidebar", ragginSidebar)
  );
  const ragdisposable = vscode8.commands.registerCommand(
    "raggin.generateComment",
    GenerateCommentCommand
  );
  const ragFunctionDisposable = vscode8.commands.registerCommand(
    "raggin.gfunctionComment",
    GfunctionCommentCommand
  );
  const ragEntireCodeBaseDisposable = vscode8.commands.registerCommand(
